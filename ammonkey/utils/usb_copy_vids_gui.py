...
async def _with_timeout(fn, timeout=_DISK_TIMEOUT, default=None):
    """Run *fn()* in the default executor; return *default* on timeout/error."""
    try:
...
async def _cancellable_copy2(src: Path, dst: Path, cancel: asyncio.Event,
                             chunk: int = _COPY_CHUNK):
    """``shutil.copy2`` replacement that checks *cancel* between chunks.

    File I/O is offloaded to the default executor so the event loop stays
    responsive.
    """
    loop = asyncio.get_running_loop()

    def _open_pair():
        return open(src, 'rb'), open(dst, 'wb')

    fin, fout = await loop.run_in_executor(None, _open_pair)
    try:
        while True:
            if cancel.is_set():
                raise InterruptedError("copy cancelled")
            buf = await loop.run_in_executor(None, fin.read, chunk)
            if not buf:
                break
            await loop.run_in_executor(None, fout.write, buf)
    finally:
        await loop.run_in_executor(None, fin.close)
        await loop.run_in_executor(None, fout.close)
    await loop.run_in_executor(None, shutil.copystat, src, dst)
...
class CardPanel(ft.Container):
    ...
    # -- refresh / query --
    async def refresh(self, date_str: str, drive_map: dict[str, str]):
        ...
        file_infos = await _with_timeout(
...
class _UsbWatcher:
    """Async task that polls for Windows USB volume changes.

    Uses WMI ``Win32_VolumeChangeEvent`` via ``asyncio.to_thread``.
    ...
    * **wait_for_replug=True** – if all volumes are *already* present
      ...
      *removed*, then waits for all to come back.
    """
    ...
    def _all_present_sync(self) -> bool:
        """Synchronous check – meant to run inside ``to_thread``."""
        drives = ucv.scan_drives()
        return self._expected.issubset(set(drives.keys()))

    def _wait_for_event_sync(self, watcher):
        """Block up to 2 s for one WMI event (runs in executor)."""
        try:
    ...
    async def _run(self):
        loop = asyncio.get_running_loop()
        try:
            already_all = await loop.run_in_executor(
                None, self._all_present_sync,
            )
            ...
            def _make_watcher():
                ...
                c = _wmi.WMI()
                return c.Win32_VolumeChangeEvent.watch_for()

            watcher = await loop.run_in_executor(None, _make_watcher)

            # Phase 1: wait for a removal (when replug required)
            if already_all and self._wait_for_replug:
                lg.debug("USB watcher: all present, waiting for a removal")
                while not self._stop:
                    await loop.run_in_executor(
                        None, self._wait_for_event_sync, watcher,
                    )
...
class VideoIngestApp:
    ...
    def __init__(self, page: ft.Page):
        ...
        self._cancel_copy = asyncio.Event()
    ...
    def _build_widgets(self):
        ...
        self.btn_refresh = ft.ElevatedButton(
            ...
            on_click=lambda _: self.page.run_task(self._do_refresh),
            height=30,
            style=ft.ButtonStyle(
                padding=ft.padding.symmetric(horizontal=10, vertical=0),
            ),
        ...
        self.btn_copy = ft.FilledButton(
            ...
            on_click=lambda _: self.page.run_task(self._do_copy),
            height=30,
            style=ft.ButtonStyle(
    ...
    async def _on_date_picked(self, e):
    ...
    async def _minimize_to_watch(self, _e=None):
        """Hide window and start listening for USB events."""
        expected = set(ucv.TARGET_VID_DIRS.keys())
        currently_all = await asyncio.to_thread(
            lambda: expected.issubset(set(ucv.scan_drives().keys()))
        )
    ...
    async def _on_all_drives_connected(self):
        ...
        await self._on_date_picked(self.date_picker)
        self._safe_update()
    ...
    async def _do_refresh(self):
        ...
        drives = await _with_timeout(ucv.scan_drives, timeout=_DISK_TIMEOUT, default={})
        if drives is None:
            drives = {}
    ...
    async def _do_copy(self):
        ...
        if not base.exists():
            try:
                await asyncio.to_thread(
                    lambda: base.mkdir(parents=True, exist_ok=True)
                )
        ...
        for cw in self.card_widgets:
            ...
            await asyncio.to_thread(
                lambda d=dst_dir: d.mkdir(parents=True, exist_ok=True)
            )
        ...
        async def _copy_card(cw: CardPanel, files: list[str], dst_dir: Path) -> list[Path]:
            nonlocal done_files
            ...
            for fname in files:
                ...
                if exists:
                    ...
                    done_files += 1
                    self._show_copy_progress(done_files, total_files)
                    continue
                ...
                try:
                    await _cancellable_copy2(src, tmp, self._cancel_copy)
        ...
        for _, _, d in card_jobs:
            stale_files = await asyncio.to_thread(
                lambda dd=d: list(dd.glob(f"*{_COPY_TMP_SUFFIX}"))
            )
        ...
        # Run card copies concurrently via asyncio.gather
        tasks = [
            _copy_card(cw, sel, dst)
            for cw, sel, dst in card_jobs
        ]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for (cw, _, _), result in zip(card_jobs, results):
            if isinstance(result, BaseException):
                if not self._cancel_copy.is_set():
                    lg.error(f"Copy failed for card {cw.vol_name}: {result}")
                    errors.append(f"{cw.vol_name}: {result}")
            else:
                all_copied.extend(result)
